...
let countKeyPress = 0;
const game = new Game();
if (gameRefs.score) gameRefs.score.innerText = game.getScore();
if (gameRefs.best) gameRefs.best.innerText = game.getBestScore();
...
gameRefs.start.addEventListener("click", ()=>{
    ...
    if (gameRefs.start.textContent === "Restart") {
        if (game.getStatus() === "win") gameRefs.messageWin.classList.add("hidden");
        if (game.getStatus() === "lose") gameRefs.messageLose.classList.add("hidden");
        game.restart();
